...
class TrapezoidalMembershipFunction(torch.nn.Module):
    ...
    def extra_repr(self) -> str:
        repr = ""
        if self.b == -TrapezoidalMembershipFunction.pseudo_infinity:
            repr += "-inf, "
        else:
            repr += f"{self.a}, {self.b}, "

        if self.c == TrapezoidalMembershipFunction.pseudo_infinity:
            repr += "inf"
        else:
            repr += f"{self.c}, {self.d}"
    ...
    def get_left_open(c: float, d: float):
        return TrapezoidalMembershipFunction(-(TrapezoidalMembershipFunction.pseudo_infinity + 1),
                                             -TrapezoidalMembershipFunction.pseudo_infinity,
    ...
    def get_right_open(a: float, b: float):
        return TrapezoidalMembershipFunction(a, b,
                                             TrapezoidalMembershipFunction.pseudo_infinity,
                                             TrapezoidalMembershipFunction.pseudo_infinity + 1,
                                             freeze_right=True)
